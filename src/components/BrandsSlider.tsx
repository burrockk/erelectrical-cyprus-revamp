    ...
    <section id="brands" className="py-20 bg-white">
      <style>{`
        @keyframes scroll-brands {
          ...
          100% {
            transform: translateX(calc(-272px * ${brands.length}));
          }
        }
        ...
        <div className="relative overflow-hidden">
          ...
          >
            {duplicatedBrands.map((brand, index) => (
              <div 
                key={index}
                className="flex-shrink-0 w-64 flex items-center justify-center h-32 bg-muted rounded-lg transition-all duration-300 group cursor-pointer p-4 hover:bg-primary hover:text-white"
              >
